...
## 文件大小指导
- 保持手写源码文件足够小，便于审查和推理。
- 新建手写源码文件通常应保持在 600 行以内。
- 落地页样式表、站点模板和页面生成脚本通常应保持在 500 行以内，并通过拆出聚焦的局部样式、辅助模块、内容模块或更小的生成单元来控制体量。
- 不要新增超过 800 行的手写源码文件，除非在变更总结中明确记录充分理由。
- 对已经超过这些阈值的手写源码文件做编辑时，应避免继续增加其行数。如果本次改动是非平凡变更，则至少抽出一个有内聚性的辅助单元、局部样式、内容模块或脚本拆分，除非这样做会引入与当前任务无关的额外扰动。
- 生成文件、锁文件、快照、夹具数据、第三方参考文件，以及迁移类产物均豁免。
