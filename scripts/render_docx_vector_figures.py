from __future__ import annotations

import argparse
from pathlib import Path
import sys

ROOT_DIR = Path(__file__).resolve().parents[1]
if str(ROOT_DIR) not in sys.path:
    sys.path.insert(0, str(ROOT_DIR))

from scripts.docx_figures import build_figure_inventory
from scripts.docx_figures.vector_media import render_vector_blip_png

DEFAULT_DOCX = Path(
    "resources/Exploration and Exploitation of Petroleum Resources in West Africa (Matt Edited).docx"
)
DEFAULT_SUMMARY = Path("src/SUMMARY.md")
DEFAULT_CHAPTERS_DIR = Path("src/chapters")
DEFAULT_OUTPUT_DIR = Path("src/images")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Render DOCX EMF vector-media figures into PNG assets."
    )
    parser.add_argument("--docx", default=str(DEFAULT_DOCX))
    parser.add_argument("--summary", default=str(DEFAULT_SUMMARY))
    parser.add_argument("--chapters-dir", default=str(DEFAULT_CHAPTERS_DIR))
    parser.add_argument("--output-dir", default=str(DEFAULT_OUTPUT_DIR))
    parser.add_argument(
        "--figures",
        nargs="*",
        type=int,
        help="Optional subset of figure numbers to render.",
    )
    return parser.parse_args()


def main() -> int:
    args = parse_args()
    inventory = build_figure_inventory(
        docx_path=Path(args.docx),
        chapters_dir=Path(args.chapters_dir),
        summary_path=Path(args.summary),
    )
    requested = set(args.figures or [])
    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    rendered = 0
    for record in inventory:
        if requested and record.number not in requested:
            continue
        emf_targets = [
            target
            for target in record.objects.blip_targets
            if target.lower().endswith((".emf", ".wmf"))
        ]
        if not emf_targets:
            continue
        width, height, png_bytes = render_vector_blip_png(Path(args.docx), emf_targets[0])
        output_path = output_dir / f"figure-{record.number:03d}.png"
        output_path.write_bytes(png_bytes)
        rendered += 1
        print(f"Rendered Figure {record.number} -> {output_path} ({width}x{height})")

    print(f"Rendered {rendered} vector-media figure(s).")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
