...
check_not_contains public/index.html '<h2>Search Upstream Atlas</h2>'
check_contains public/index.html 'class="homepage-search-shell"'
...
check_contains public/index.html 'class="summary-card-link" href="/chapters/">View all updates'
check_contains public/index.html 'class="summary-card-link" href="/#topics">View all topics'
check_not_contains public/index.html 'class="summary-card-link" href="/chapters/">Learn more'
...
check_contains "$LANDING_CSS_ASSERT_FILE" '.map-overview-grid::before {'
check_contains "$LANDING_CSS_ASSERT_FILE" '.west-africa-map-canvas {'
check_contains "$LANDING_CSS_ASSERT_FILE" '.west-africa-map-card {'
check_contains "$LANDING_CSS_ASSERT_FILE" '.west-africa-map-panel-image {'
check_contains "$LANDING_CSS_ASSERT_FILE" 'aspect-ratio: 1044 / 260;'
check_contains "$LANDING_CSS_ASSERT_FILE" 'width: min(100%, 1044px);'
check_contains "$LANDING_CSS_ASSERT_FILE" 'background: transparent;'
...
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");function block(selector){const start=css.indexOf(selector);if(start===-1){console.error(`Expected selector block: ${selector}`);process.exit(1);}const end=css.indexOf("}",start);if(end===-1){console.error(`Expected closing brace for selector block: ${selector}`);process.exit(1);}return css.slice(start,end+1);}const gradientBlock=block(".map-overview-grid::before {");for(const expected of ["var(--page-bg) 0%,","var(--page-bg) 22%,","rgba(216, 230, 246, 1) 100%"]){if(!gradientBlock.includes(expected)){console.error(`Expected .map-overview-grid::before to include ${expected}.`);process.exit(1);}}const hotspotBlock=block(".west-africa-map-hotspot {");for(const expected of ["width: max(var(--west-africa-map-hotspot-width), 1.9rem);","height: max(var(--west-africa-map-hotspot-height), 1.25rem);","overflow: visible;"]){if(!hotspotBlock.includes(expected)){console.error(`Expected .west-africa-map-hotspot to include ${expected}.`);process.exit(1);}}const hotspotAfterBlock=block(".west-africa-map-hotspot::after {");for(const expected of ["left: 50%;","top: 50%;","width: calc(var(--west-africa-map-hotspot-width) + 0.28rem);","height: calc(var(--west-africa-map-hotspot-height) + 0.28rem);","transform: translate(-50%, -50%);"]){if(!hotspotAfterBlock.includes(expected)){console.error(`Expected .west-africa-map-hotspot::after to include ${expected}.`);process.exit(1);}}const hotspotFlagBlock=block(".west-africa-map-hotspot-flag {");for(const expected of ["left: 50%;","top: 50%;","width: var(--west-africa-map-hotspot-width);","height: var(--west-africa-map-hotspot-height);","transform: translate(-50%, -50%);"]){if(!hotspotFlagBlock.includes(expected)){console.error(`Expected .west-africa-map-hotspot-flag to include ${expected}.`);process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const start=css.indexOf(".map-overview-copy .button.button-primary {");if(start===-1){console.error("Expected .map-overview-copy .button.button-primary rule block in expanded landing CSS.");process.exit(1);}const end=css.indexOf("}",start);if(end===-1){console.error("Expected closing brace for .map-overview-copy .button.button-primary rule block.");process.exit(1);}const block=css.slice(start,end+1);for(const expected of ["height: 2.28rem;","min-height: 2.28rem;","gap: 0.48rem;","padding: 0 0.9rem 0 1rem;","font-size: 0.84rem;","line-height: 1;"]){if(!block.includes(expected)){console.error(`Expected .map-overview-copy .button.button-primary to include ${expected}.`);process.exit(1);}}'
...
check_contains "$LANDING_CSS_ASSERT_FILE" '.section-heading.section-heading-topic-reference {'
check_contains "$LANDING_CSS_ASSERT_FILE" '.topic-grid-reference {'
...
check_contains "$LANDING_CSS_ASSERT_FILE" '.summary-card-edition h3 {'
check_contains "$LANDING_CSS_ASSERT_FILE" '.section-heading.section-heading-wide {'
check_contains "$LANDING_CSS_ASSERT_FILE" 'margin-bottom: 0;'
...
if (search.includes("padding-top: 2.55rem;")) {
  console.error("Expected .section-search-surface to stop using desktop padding-top: 2.55rem.");
  process.exit(1);
}
...
const searchHeadingWrap = block(".section-search-surface .section-heading {");
if (!searchHeadingWrap.includes("margin-bottom: 12px;")) {
  console.error("Expected .section-search-surface .section-heading to use margin-bottom: 12px.");
  process.exit(1);
}
...
const mobileSegment =
  mobileStart === -1 ? "" : css.slice(mobileStart, mobileEnd === -1 ? undefined : mobileEnd);
const mobileSearchStart = mobileSegment.indexOf(".section-search-surface {");
const mobileSearchEnd =
  mobileSearchStart === -1 ? -1 : mobileSegment.indexOf("}", mobileSearchStart);
...
const topicHeading = block(".section-heading.section-heading-topic-reference {");
for (const expected of ["gap: 0;","margin-bottom: 1.15rem;"]) {
  if (!topicHeading.includes(expected)) {
    console.error(`Expected .section-heading.section-heading-topic-reference to include ${expected}.`);
    process.exit(1);
  }
}

const topicGrid = block(".topic-grid-reference {");
for (const expected of ["grid-template-columns: repeat(6, minmax(0, 1fr));","align-items: stretch;"]) {
  if (!topicGrid.includes(expected)) {
    console.error(`Expected .topic-grid-reference to include ${expected}.`);
    process.exit(1);
  }
}
...
for (const expected of ["gap: 0.6rem;","padding: 0.22rem 0.98rem 0.18rem;"]) {
...
if (!heroStatDivider.includes("border-left: 1px solid rgba(255, 255, 255, 0.1);")) {
  console.error("Expected .hero-stat-card + .hero-stat-card to restore clearer vertical separation.");
  process.exit(1);
}

const heroStatIcon = block(".hero-stat-icon {");
for (const expected of ["width: 2.72rem;","height: 2.72rem;","margin-bottom: 0.56rem;","--homepage-inline-icon-stroke: 2;"]) {
...
for (const expected of ["gap: 0.16rem;","line-height: 1.3;"]) {
...
for (const expected of ["background: rgba(9, 27, 48, 0.32);","border-color: rgba(255, 255, 255, 0.5);","box-shadow: inset 0 0 0 1px rgba(255, 255, 255, 0.12);"]) {
...
const decisionStripSection = block(".section.section-platform.decision-strip {");
if (!decisionStripSection.includes("background: transparent;")) {
  console.error("Expected .section.section-platform.decision-strip to remove the inherited platform background.");
  process.exit(1);
}
...
const firstUpdateDateStart = css.indexOf(".update-date {");
const updateDateStart =
  firstUpdateDateStart === -1 ? -1 : css.indexOf(".update-date {", firstUpdateDateStart + 1);
if (updateDateStart === -1) {
  console.error("Expected the standalone .update-date rule block in expanded landing CSS.");
  process.exit(1);
}
const updateDateEnd = css.indexOf("}", updateDateStart);
if (updateDateEnd === -1) {
  console.error("Expected closing brace for the standalone .update-date rule block.");
  process.exit(1);
}
...
const editionCard = block(".edition-card {");
for (const expected of ["grid-template-columns: minmax(0, 1fr) 7.2rem;","gap: 0.96rem;"]) {
  if (!editionCard.includes(expected)) {
    console.error(`Expected .edition-card to include ${expected}.`);
    process.exit(1);
  }
}
...
check_contains "$LANDING_CSS_ASSERT_FILE" '.homepage-search-shell {'
check_contains "$LANDING_CSS_ASSERT_FILE" 'grid-template-columns: auto minmax(0, 1fr);'
check_contains "$LANDING_CSS_ASSERT_FILE" 'width: min(75rem, calc(100% - 4rem));'
check_contains "$LANDING_CSS_ASSERT_FILE" 'justify-content: center;'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");function block(selector){const start=css.indexOf(selector);if(start===-1){console.error(`Expected selector block: ${selector}`);process.exit(1);}const end=css.indexOf("}",start);if(end===-1){console.error(`Expected closing brace for selector block: ${selector}`);process.exit(1);}return css.slice(start,end+1);}const searchForm=block(".homepage-search-form {");for(const expected of ["width: min(75rem, calc(100% - 4rem));","margin: 0 auto;"]){if(!searchForm.includes(expected)){console.error(`Expected .homepage-search-form to include ${expected}.`);process.exit(1);}}const searchShell=block(".homepage-search-shell {");for(const expected of ["min-height: 2.96rem;","border-radius: 999px;","border: 1px solid rgba(126, 144, 175, 0.36);"]){if(!searchShell.includes(expected)){console.error(`Expected .homepage-search-shell to include ${expected}.`);process.exit(1);}}const searchInput=block(".homepage-search-form input {");for(const expected of ["border: 0;","background: transparent;","font-size: 1.08rem;"]){if(!searchInput.includes(expected)){console.error(`Expected .homepage-search-form input to include ${expected}.`);process.exit(1);}}const searchCancel=block(".homepage-search-form input::-webkit-search-cancel-button {");if(!searchCancel.includes("appearance: none;")){console.error("Expected the homepage search input to suppress the native WebKit cancel button.");process.exit(1);}'
...
for (const expected of ["--stakeholder-icon-size: 55px;"]) {
...
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const match=css.match(/@media \(max-width: 767px\) \{[\s\S]*?\.section-search-surface \.homepage-search-form \{[\s\S]*?width: min\(75rem, calc\(100% - 2rem\)\);/);if(!match){console.error("Expected narrow homepage search CSS to reserve a full 1rem gutter on both sides below 768px.");process.exit(1);}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");function block(selector){const start=css.indexOf(selector);if(start===-1){console.error(`Expected selector block: ${selector}`);process.exit(1);}const end=css.indexOf("}",start);if(end===-1){console.error(`Expected closing brace for selector block: ${selector}`);process.exit(1);}return css.slice(start,end+1);}const chipBlock=block(".section-search-surface .search-scope-chip {");for(const expected of ["box-sizing: border-box;","justify-content: flex-start;","text-align: left;"]){if(!chipBlock.includes(expected)){console.error(`Expected .section-search-surface .search-scope-chip to include ${expected} so multi-row filters align to a stable left edge.`);process.exit(1);}}const chipLabelBlock=block(".section-search-surface .search-scope-chip-label {");for(const expected of ["white-space: normal;","line-height: 1.2;"]){if(!chipLabelBlock.includes(expected)){console.error(`Expected .section-search-surface .search-scope-chip-label to include ${expected} so long filter labels can wrap inside equal-width tracks.`);process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const start=css.indexOf("@media (max-width: 699px) {");const end=css.indexOf("@media (min-width: 700px) and (max-width: 767px) {",start);if(start===-1||end===-1){console.error("Expected phone search-scope media block boundaries in expanded landing CSS.");process.exit(1);}const block=css.slice(start,end);for(const expected of ["display: grid;","grid-template-columns: repeat(2, minmax(0, 1fr));","width: min(27rem, calc(100% - 2rem));","margin-left: auto;","margin-right: auto;","width: 100%;"]){if(!block.includes(expected)){console.error("Expected phone search-scope CSS to include "+expected+" so narrow screens keep the same 1rem side gutter as the search input.");process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const marker="@media (min-width: 700px) and (max-width: 767px) {";const start=css.lastIndexOf(marker);const next=css.indexOf("\n@media",start + marker.length);if(start===-1||next===-1){console.error("Expected 700px-767px search-scope media block in expanded landing CSS.");process.exit(1);}const block=css.slice(start,next);for(const expected of ["display: flex;","flex-wrap: wrap;","justify-content: center;","gap: 0.72rem;","width: calc(100% - 2rem);","margin-left: auto;","margin-right: auto;","flex: 0 0 calc((100% - 1.44rem) / 3);","width: calc((100% - 1.44rem) / 3);","white-space: nowrap;"]){if(!block.includes(expected)){console.error("Expected 700px-767px search-scope CSS to include "+expected+" so large phones align the chip group to the same 1rem side gutter as the search input.");process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const start=css.indexOf("@media (min-width: 768px) and (max-width: 959px) {");if(start===-1){console.error("Expected 768px-959px search-scope media block in expanded landing CSS.");process.exit(1);}const next=css.indexOf("\n@media",start + 1);const block=next===-1?css.slice(start):css.slice(start,next);for(const expected of ["display: flex;","flex-wrap: wrap;","justify-content: center;","gap: 0.72rem;","width: calc(100% - 4rem);","margin-left: auto;","margin-right: auto;","flex: 0 0 calc((100% - 1.44rem) / 3);","width: calc((100% - 1.44rem) / 3);","white-space: nowrap;"]){if(!block.includes(expected)){console.error("Expected 768px-959px search-scope CSS to include "+expected+" so tablets align the chip group to the same 2rem side gutter as the search input.");process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const start=css.indexOf("@media (min-width: 960px) and (max-width: 1679px) {");if(start===-1){console.error("Expected wide-tablet-to-desktop search-scope media block in expanded landing CSS.");process.exit(1);}const next=css.indexOf("\n@media",start + 1);const block=next===-1?css.slice(start):css.slice(start,next);for(const expected of ["display: grid;","grid-template-columns: repeat(4, minmax(0, 1fr));","width: min(75rem, calc(100% - 4rem));","margin-left: auto;","margin-right: auto;","width: 100%;","white-space: nowrap;"]){if(!block.includes(expected)){console.error("Expected 960px-1679px search-scope CSS to include "+expected+" so the equal-width two-row chip block stays aligned with the search input until true wide desktop widths can absorb the single-row pills.");process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const marker="@media (min-width: 1680px) {";const start=css.lastIndexOf(marker);if(start===-1){console.error("Expected wide-desktop search-scope media block in expanded landing CSS.");process.exit(1);}const next=css.indexOf("\n@media",start + marker.length);const block=next===-1?css.slice(start):css.slice(start,next);for(const expected of [".section-search-surface .search-scope-grid {","width: min(75rem, calc(100% - 4rem));","margin-left: auto;","margin-right: auto;"]){if(!block.includes(expected)){console.error("Expected >=1680px search-scope CSS to include "+expected+" so the single-row desktop pills keep the same shared content width as the search input.");process.exit(1);}}'
check_not_contains "$LANDING_CSS_ASSERT_FILE" '@media (min-width: 700px) and (max-width: 1279px) {'
check_not_contains "$LANDING_CSS_ASSERT_FILE" '@media (min-width: 1120px) and (max-width: 1279px) {'
check_not_contains "$LANDING_CSS_ASSERT_FILE" 'grid-template-columns: repeat(4, max-content);'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const match=css.match(/@media \(min-width: 768px\) and \(max-width: 1119px\) \{[\s\S]*?\.site-header \.brand-mark \{[\s\S]*?\.site-header \.brand-mark-image-compact \{[\s\S]*?display: none;[\s\S]*?\}/);if(!match){console.error("Expected landing tablet header logo media block in expanded landing CSS.");process.exit(1);}const block=match[0];for(const expected of [".site-header .brand-mark {","width: auto;","height: 44px;","flex: 0 0 auto;",".site-header .brand-mark-image-full {","width: auto;","height: 36px;",".site-header .brand-mark-image-compact {","display: none;"]){if(!block.includes(expected)){console.error("Expected landing tablet header logo CSS to include "+expected);process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const start=css.indexOf("@media (min-width: 768px) and (max-width: 860px) {");const end=css.indexOf("@media (min-width: 861px) and (max-width: 1119px) {",start);if(start===-1||end===-1){console.error("Expected tablet-portrait landing media block boundaries in expanded landing CSS.");process.exit(1);}const block=css.slice(start,end);for(const expected of [".hero-panel-v2 .hero-content {","padding: 2.1rem 0 2.35rem;",".hero-copy-block-v2 {","display: flex;","flex-direction: column;","width: min(100%, 40rem);","max-width: 40rem;",".hero-panel-v2 .hero-copy-block h1 {","max-width: 13.4ch;","font-size: clamp(2.18rem, 4.25vw, 2.58rem);",".hero-panel-v2 .hero-copy {","order: 3;","max-width: 36rem;","font-size: 0.96rem;","line-height: 1.68;",".hero-panel-v2 .hero-actions {","order: 4;","flex-direction: column;","align-items: flex-start;","width: min(100%, 31rem);",".hero-panel-v2 .hero-actions .button {","width: 100%;",".hero-panel-v2 .hero-stat-grid {","order: 5;","grid-template-columns: repeat(3, minmax(0, 1fr));","max-width: min(100%, 43rem);",".hero-panel-v2 .hero-stat-card {","border-radius: 0.95rem;","background: rgba(9, 27, 48, 0.28);",".hero-panel-v2 .hero-stat-value {","font-size: clamp(1.9rem, 3.4vw, 2.16rem);",".stakeholder-grid {","grid-template-columns: repeat(3, minmax(0, 1fr));",".stakeholder-card {","width: 100%;","min-height: 8.05rem;"]){if(!block.includes(expected)){console.error("Expected tablet-portrait landing hero CSS to include "+expected);process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const start=css.indexOf("@media (min-width: 861px) and (max-width: 1119px) {");const end=css.indexOf("@media (min-width: 768px) and (max-width: 1119px) {",start);if(start===-1||end===-1){console.error("Expected wide-tablet landing media block boundaries in expanded landing CSS.");process.exit(1);}const block=css.slice(start,end);for(const expected of [".hero-panel-v2 .hero-content {","padding: 2.2rem 0 2.5rem;",".hero-copy-block-v2 {","display: flex;","flex-direction: column;","width: min(100%, clamp(46rem, 71vw, 52rem));","max-width: 52rem;",".hero-panel-v2 .hero-copy-block h1 {","max-width: 23.5ch;","font-size: clamp(2.24rem, 3.25vw, 2.74rem);",".hero-panel-v2 .hero-copy {","order: 3;","max-width: clamp(42rem, 62vw, 47rem);","font-size: 0.97rem;","line-height: 1.7;",".hero-panel-v2 .hero-actions {","order: 4;","flex-direction: column;","align-items: flex-start;","width: min(100%, clamp(33rem, 55vw, 38rem));","max-width: 38rem;",".hero-panel-v2 .hero-actions .button {","width: 100%;",".hero-panel-v2 .hero-stat-grid {","order: 5;","grid-template-columns: repeat(3, minmax(0, 1fr));","max-width: min(100%, clamp(49rem, 72vw, 54rem));",".hero-panel-v2 .hero-stat-card {","border-radius: 0.95rem;","background: rgba(9, 27, 48, 0.22);",".hero-panel-v2 .hero-stat-value {","font-size: clamp(1.96rem, 2.75vw, 2.2rem);",".stakeholder-grid {","grid-template-columns: repeat(3, minmax(0, 1fr));",".stakeholder-card {","width: 100%;","min-height: 7.95rem;"]){if(!block.includes(expected)){console.error("Expected wide-tablet landing hero CSS to include "+expected);process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const marker="@media (min-width: 768px) and (max-width: 1119px) {";const start=css.lastIndexOf(marker);if(start===-1){console.error("Expected tablet summary landing media block boundaries in expanded landing CSS.");process.exit(1);}const next=css.indexOf("\n@media",start + marker.length);const block=next===-1?css.slice(start):css.slice(start,next);for(const expected of [".section-summary-modules .summary-grid {","grid-template-columns: repeat(2, minmax(0, 1fr));","align-items: start;",".section-summary-modules .summary-card {","min-height: auto;","align-self: start;",".section-summary-modules .summary-card-copy {","max-width: none;"]){if(!block.includes(expected)){console.error("Expected tablet summary landing CSS to include "+expected);process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const match=css.match(/@media \(min-width: 1120px\) and \(max-width: 1679px\) \{[\s\S]*?\.site-header \.brand-mark-image-full \{[\s\S]*?height: 36px;[\s\S]*?\}/);if(!match){console.error("Expected landing small-desktop header media block in expanded landing CSS.");process.exit(1);}const block=match[0];for(const expected of [".site-header .brand-mark-image-full {","width: auto;","height: 36px;"]){if(!block.includes(expected)){console.error("Expected landing small-desktop header CSS to include "+expected);process.exit(1);}}'
...
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const compact320Start=css.indexOf("@media (max-width: 320px) {");const compact320End=css.indexOf("@media (min-width: 321px) and (max-width: 360px) {",compact320Start);if(compact320Start===-1||compact320End===-1){console.error("Expected <=320px landing micro-adjustment media block in expanded landing CSS.");process.exit(1);}const block=css.slice(compact320Start,compact320End);for(const expected of [".hero-panel-v2 .hero-copy-block h1 {","max-width: 12.4ch;","font-size: clamp(1.68rem, 8.35vw, 1.88rem);"]){if(!block.includes(expected)){console.error("Expected <=320px landing CSS to include "+expected+" so the hero title regains a safe right gutter.");process.exit(1);}}'
LANDING_CSS_ASSERT_FILE="$LANDING_CSS_ASSERT_FILE" node -e 'const fs=require("fs");const css=fs.readFileSync(process.env.LANDING_CSS_ASSERT_FILE,"utf8");const largePhoneStart=css.indexOf("@media (min-width: 700px) and (max-width: 767px) {");const largePhoneEnd=css.indexOf("@media (max-width: 480px) {",largePhoneStart);if(largePhoneStart===-1||largePhoneEnd===-1){console.error("Expected 700px-767px landing large-phone media block in expanded landing CSS.");process.exit(1);}const block=css.slice(largePhoneStart,largePhoneEnd);for(const expected of [".country-grid-v2,",".topic-grid,",".summary-grid,",".topic-grid-compact {","grid-template-columns: repeat(2, minmax(0, 1fr));",".section-summary-modules .summary-grid {","grid-template-columns: repeat(2, minmax(0, 1fr));"]){if(!block.includes(expected)){console.error("Expected 700px-767px landing CSS to include "+expected+" so upper-bound phone widths condense homepage grids before tablet mode.");process.exit(1);}}'
...
check_contains assets/css/landing.css '@import "./landing.search-scope-layout.css";'
check_not_contains scripts/generate-chapters-page.mjs 'replaceAll('
check_not_contains scripts/shared/homepage-outline-icons.mjs 'replaceAll('
...
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'reader-layout'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'reader-main'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'reader-outline'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'reader-article'
check_exists public/book/chapters/chapter-10-petroleum-data-management-in-west-africa.html
check_contains public/book/chapters/chapter-10-petroleum-data-management-in-west-africa.html 'meta name="robots" content="noindex,follow"'
check_contains public/book/chapters/chapter-10-petroleum-data-management-in-west-africa.html 'rel="canonical" href="https://upstreamatlas.com/book/chapters/chapter-07-petroleum-data-management-in-west-africa.html"'
check_contains public/book/chapters/chapter-10-petroleum-data-management-in-west-africa.html 'const target = new URL("./chapter-07-petroleum-data-management-in-west-africa.html", window.location.href);'
check_not_contains public/book-sitemap.xml 'https://upstreamatlas.com/book/chapters/chapter-10-petroleum-data-management-in-west-africa.html'
check_contains public/book-sitemap.xml 'https://upstreamatlas.com/book/chapters/chapter-07-petroleum-data-management-in-west-africa.html'
...
check_contains public/fr/index.html '<h3>Recherche dans Upstream Atlas</h3>'
...
check_contains editions/en/content/SUMMARY.md 'chapters/chapter-09-west-african-fiscal-regimes.md'
...
check_contains public/book/chapters/list-of-figures.html 'chapter-09-west-african-fiscal-regimes.html#figure-79'
...
check_contains public/book/chapters/table-of-contents.html 'class="reference-index-link" href="chapter-09-west-african-fiscal-regimes.html#94--state-contractor-cash-flow-analysis">9.4 State-Contractor Cash Flow Analysis</a>'
...
check_contains public/book/chapters/list-of-tables.html 'chapter-09-west-african-fiscal-regimes.html#table-11'
...
check_contains public/book/chapters/list-of-equations.html 'chapter-09-west-african-fiscal-regimes.html#formula-9-3'
...
check_contains editions/en/content/chapters/chapter-09-west-african-fiscal-regimes.md '<table>'
check_contains editions/en/content/chapters/chapter-09-west-african-fiscal-regimes.md 'Table 20 Principal Fiscal Elements'
...
check_not_contains editions/en/content/chapters/chapter-09-west-african-fiscal-regimes.md '&amp;amp;'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html '<table>'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html '<thead>'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html '<tbody>'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'Table 20 Principal Fiscal Elements'
...
check_not_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html '&amp;amp;'
check_contains editions/en/content/chapters/chapter-09-west-african-fiscal-regimes.md 'Table 11 Summary of Ad Valorem Royalty Rates Applied in Selected West African Countries.'
check_contains editions/en/content/chapters/chapter-09-west-african-fiscal-regimes.md 'Table 12 Summary of Profit Oil Sharing Mechanisms and Government Profit Oil Entitlements in Selected West African Countries.'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html '<p>Table 11 Summary of Ad Valorem Royalty Rates Applied in Selected West African Countries.</p>'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html '<p>Table 12 Summary of Profit Oil Sharing Mechanisms and Government Profit Oil Entitlements in Selected West African Countries.</p>'
...
check_contains editions/en/content/chapters/chapter-09-west-african-fiscal-regimes.md 'data-equation-label="9.1"'
check_contains editions/en/content/chapters/chapter-09-west-african-fiscal-regimes.md 'data-equation-label="9.2"'
check_contains editions/en/content/chapters/chapter-09-west-african-fiscal-regimes.md 'data-equation-label="9.3"'
check_contains editions/en/content/chapters/chapter-09-west-african-fiscal-regimes.md 'class="table-12-h-factor-cell"'
...
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'data-equation-label="9.1"'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'data-equation-label="9.2"'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'data-equation-label="9.3"'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'class="book-formula-bridge">or<'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'H = 1.626'
...
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'class="reader-sidebar-projection"'
check_contains public/book/chapters/chapter-09-west-african-fiscal-regimes.html 'class="sidebar book-sidebar-shell book-sidebar-shell--projected"'
