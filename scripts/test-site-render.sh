...
node -e 'const fs=require("fs");const html=fs.readFileSync("public/book/chapters/chapter-04-comparative-study-of-tax-regimes-in-selected-west-african-countries.html","utf8");const normalize=(value)=>value.replace(/<[^>]+>/g," ").replace(/&nbsp;/g," ").replace(/\u00a0/g," ").replace(/&minus;/g,"-").replace(/[−–]/g,"-").replace(/&times;/g,"x").replace(/&deg;/g,"°").replace(/\s+/g," ").trim();const text=normalize(html);const labels=[...html.matchAll(/data-equation-label=\"([^\"]+)\"/g)].map((match)=>match[1]);if(labels.join(",")!=="4.1,4.2"){console.error(`Expected chapter 4 primary equation labels 4.1,4.2 but found ${labels.join(",")}`);process.exit(1);}for(const expected of ["Post Royalty Revenue = Gross Revenue - Royalty","Oil Profit = Gross Revenue - Royalty - Recoverable Costs","R-Factor=Cumulative Net Revenue/Cumulative Costs"]){if(!text.includes(expected)){console.error(`Expected chapter 4 formula content for: ${expected}`);process.exit(1);}}const splitMatch=html.match(/<section class=\"formula-group formula-group--split formula-group--oil-profit\"[\s\S]*?<\/section>/);if(!splitMatch){console.error("Expected chapter 4 split oil-profit formula group");process.exit(1);}const split=splitMatch[0];const normalizedSplit=normalize(split);for(const expected of ["class=\"formula-split-divider\"","class=\"book-formula-bridge\">or<"]){if(!split.includes(expected)){console.error(`Expected split oil-profit structure for: ${expected}`);process.exit(1);}}for(const expected of ["Oil Profit = Revenue Post Royalty - Recoverable Costs","Oil Profit = Gross Revenue - Royalty - Recoverable Costs"]){if(!normalizedSplit.includes(expected)){console.error(`Expected split oil-profit structure for: ${expected}`);process.exit(1);}}const panelMatch=html.match(/<section class=\"formula-panel formula-panel--r-factor\"[\s\S]*?<\/section>/);if(!panelMatch){console.error("Expected chapter 4 grouped R-factor formula panel");process.exit(1);}const panel=panelMatch[0];for(const label of ["a)","b)","c)","d)"]){if(!panel.includes(`data-formula-label=\"${label}\"`)){console.error(`Expected chapter 4 R-factor label ${label}`);process.exit(1);}}const normalizedPanel=normalize(panel);for(const formula of ["R-Factor=Cumulative Revenue/Cumulative Cost","R-Factor = (Cumulative Revenue - Cumulative Opex) / Cumulative Capex","R-Factor = (Cumulative Revenues - Cumulative Profits) / (Cumulative Investments + Cumulative Opex)","R-Factor=Cumulative Net Revenue/Cumulative Costs"]){if(!normalizedPanel.includes(formula)){console.error(`Expected grouped R-factor panel content for: ${formula}`);process.exit(1);}}for(const forbidden of ["Equation 4.3","Equation 4.4","Formula 4.3"]){if(text.includes(forbidden)){console.error(`Did not expect extra chapter 4 primary equation label: ${forbidden}`);process.exit(1);}}'
